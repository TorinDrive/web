<template>
  <div class="py-3 bg-slate-900 static shadow-2xl">
  </div>
  ...
  <header class="bg-white flex items-center justify-between px-6 md:px-8 py-4 shadow-md relative">
    ...
    <nav class="hidden md:flex items-center text-gray-600 font-semibold gap-3">
      <router-link to="/elevators/home" class="hover:underline">
        Home
      </router-link>
      <div class="w-0.5 h-6 bg-gray-400 mx-2 rounded"></div>
    ...
    <button class="hidden md:block bg-gray-600 text-white px-4 py-2 rounded-full hover:bg-gray-700 transition text-sm cursor-pointer">
      Simular Orçamento
    </button>
    ...
    >
      <nav v-if="isMobileMenuOpen" class="absolute top-full left-0 w-full bg-white shadow-lg md:hidden z-20 p-4">
        <div class="flex flex-col gap-4 text-gray-700 font-semibold">
          <router-link to="/elevators/home" @click="closeMobileMenu" class="hover:text-gray-900">Home</router-link>
          
          <div>
            <button @click="toggleProdutosMenu" class="w-full flex justify-between items-center hover:text-gray-900">
              <span>Produtos</span>
            ...
            <div v-if="isProdutosMenuOpen" class="mt-2 pl-4 flex flex-col gap-2 font-normal">
              <router-link to="/elevators/motors" @click="closeMobileMenu" class="hover:text-gray-900">Motores</router-link>
              <router-link to="/elevators/components" @click="closeMobileMenu" class="hover:text-gray-900">Componentes</router-link>
            </div>
          </div>

          <router-link to="/clientes" @click="closeMobileMenu" class="hover:text-gray-900">Clientes</router-link>
          <router-link to="/catalogo" @click="closeMobileMenu" class="hover:text-gray-900">Catálogo</router-link>
          <router-link to="/contato" @click="closeMobileMenu" class="hover:text-gray-900">Contatos</router-link>
          
          <router-link to="/orcamento" @click="closeMobileMenu" class="hover:text-gray-900">Simulação de Orçamento</router-link>

          <hr class="my-2"/>
          
          <button @click="closeMobileMenu" class="bg-gray-600 text-white px-4 py-2 rounded-full hover:bg-gray-700 transition text-sm cursor-pointer w-full">
            Simular Orçamento
          </button>
        </div>
      </nav>
